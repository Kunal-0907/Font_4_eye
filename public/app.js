...
// used for timer 
setInterval(() => {
    // if webCam is off then we don't want to increment timer
    if (!isWebCamOn) {
    ...
    if (timer === 0) {
        ...
        timer = 3;
        takeSnapShot(); // taking snapshot of video when timer reaches 0 and then resetting the timer to 3 sec
    } else timer--;

    timerText.innerHTML = timer; // setting timer value
}, 1000);

// ask permission from user and start webcam
...
function takeSnap() {
    ...
    // show spinner while the promise is in pending state
...
// displayFunction      -> for displaying and processing output from API
...
function displayOutput(json) {
    spinner.classList.add('hidden'); // hide the spinner when json is available
    ...
    // calculating the expected eye diameter when able to read clearly
    ...
    if (json[0].faceLandmarks.eyeLeftBottom.y - json[0].faceLandmarks.eyeLeftTop.y < expectedEye) {
        answerElement.innerHTML = ("The font size is not correct, wait");
